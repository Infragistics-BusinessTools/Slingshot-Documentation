...
Within 24 hours upon your request, you will receive an email from Slingshot. The email contains a link to download a zip file, which contains one or more JSON files with profile data. 
...
Below you will find the three possible scenarios. The export procedure steps depend on your relation to an Organization in Slingshot.
...
3. Specify the member's email address (or id, or what??) and your Organization name??? client id??? 
4. Export may take up to 24 hours. When export is finished, you will receive an email with a link to download the zip file containing the export data. 
