...
class AngularTSRule extends PlatformDetectorRule {
    match(info: FencedBlockInfo): boolean {
        ...
        if (PlatformDetectorRule.isTS(lang) &&
            ...
            !SlingshotTSRule.isDefiniteTSMatch(lang, info.code)) {
            return true;
        }
...
class SlingshotHTMLRule extends PlatformDetectorRule {
    match(info: FencedBlockInfo): boolean {
        if (!info.lang) {
            return false;
        }
    ...
    get platform(): APIPlatform {
        return APIPlatform.Slingshot;
    }
}

class SlingshotTSRule extends PlatformDetectorRule {
    match(info: FencedBlockInfo): boolean {
        if (!info.lang) {
            return false;
        }
        ...
        if (SlingshotTSRule.isDefiniteTSMatch(lang, info.code)) {
            return true;
        }
    ...
    static isDefiniteTSMatch(lang: string, code: string) {
        return PlatformDetectorRule.isTS(lang) && code.indexOf("Igs") >= 0;
    }
    get platform(): APIPlatform {
        return APIPlatform.Slingshot;
    }
