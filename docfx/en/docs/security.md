...
When you pin a file, you need to choose one of following permission types:

- **Request Access**: Members must request access to the file from the file owner. This is the default and most restrictive option. It means anyone who tries to open the file for the first time must request access from the file owner. The file owner will receive a notification, prompting them to grant or deny access.
...
<img class="responsive-img" src="../images/file-permission-types.png" alt="File permission types" class="responsive-img"/>
...
- Revoke edit permissions to the file through your cloud storage provider: [OneDrive](./analytics/datasources/supported-data-sources/onedrive.md), [Google Drive](./analytics/datasources/supported-data-sources/google-drive.md), [Dropbox](./analytics/datasources/supported-data-sources/dropbox.md), [Box](./analytics/datasources/supported-data-sources/box.md), and [SharePoint](./analytics/datasources/supported-data-sources/sharepoint.md).
...
- A user with a personal account in Slingshot
