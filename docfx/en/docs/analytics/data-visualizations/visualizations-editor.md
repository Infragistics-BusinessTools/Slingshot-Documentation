...
The _Visualizations Editor_ is where you create and edit your visualizations in Analytics. Here you will find the data from your dataset aggregated and prepared for use as well as great variety of visualizations to build with it.
...
* Start by **adding a new visualization** to an already existing dashboard. To do this, open a dashboard in [_Dashboard Edit_](~/components/analytics/dashboards/dashboards-interactions.html#view-edit-mode) mode and click/tap the _+ Visualization_ blue split button.

After that, you will be prompted
to add a new or select an existing [data source](~/components/analytics/datasources/overview.md).
...
1. **Data Section** - this section contains two panels:
  ...
  a. **Fields** - All of the available fields within your data source will appear on the left panel. Each field has an indicator that informs the users what field type each field is: *Date*, *Value*, *Text*. A search bar appears when the available fields are more than ten.
       The *plus* icon in this panel allows you to either [blend data sources](~/components/analytics/datasources/data-blending.md) or [calculate a field](fields/calculated-fields/overview.html#precalculated-fields). The *brain* icon allows you to use fields from a [BigQuery](~/components/analytics/datasources/ml-integration/bigquery-machine-learning-models.md) or [Azure](~/components/analytics/datasources/ml-integration/azure-machine-learning-models.md) *machine learning model* for your visualization.
...
2. **Settings Section** - this section allows you to customize what you want to show. Each visualization comes with its own settings.
  ...
  At the bottom of the *Settings* section you will see the option for links. This is a powerful feature that takes drill down to a whole new level. Read more in the [Dashboard Linking](~/components/analytics/dashboards/dashboard-linking.md) topic.
...
5. **Data Source** - The data source that you are currently working with will be displayed here. With a click you can change between the different sheets, tables or views within your source or change your connection to a new source entirely without having to leave the editor. See more in [Changing the Data Source for a Visualization](~/components/analytics/datasources/changing-data-source-visualization.md) topic.

Among other things, you will be able to:

  - [**Sort**](~/components/analytics/data-visualizations/fields/sort-by-field.md) & [**filter**](~/components/analytics/filters/visualization-filters.md) data.

  - [**Aggregate data**](~/components/analytics/data-visualizations/fields/field-settings.md) in the data editor.

  - **Search**, **Visualize** & [**format**](~/components/analytics/data-visualizations/fields/conditional-formatting.md) data.
