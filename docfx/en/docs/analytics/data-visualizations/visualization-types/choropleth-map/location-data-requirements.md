...
1. *Check the Visualization Editor*
    ...
    - Your field in the *Value* placeholder may not contain information for the selected locations. If you have added an additional category in the *Map Color* placeholder (see [this example](../choropleth-map/Choropleth-map.md#using-the-map-color-to-create-a-multi-color-choropleth-map)), then your *Value* field must contain information for this category. For example, there must be information about votes for each candidate, if you compare candidate votes by state.
...
If you are getting an outcome, but it's not exactly what you were expecting, please go through the entire Choropleth map documentation, including this topic, the [Choropleth Maps](choropleth-map.md), and [Working with the Choropleth Map Settings](../choropleth-map/settings-choropleth-map.md) topic.
