...
After a user finishes editing a visualization, upon closing the Visualization Editor, the $.ig.AnalyticsView's __visualizationEditorClosed__ event is fired:
...
As described in [**Loading Dashboard Files**](~/components/analytics/developer/web-sdk/using-the-server-sdk/loading-dashboards.md), there are two ways to handle how you save changes to dashboards:
  ...
  - **Server-side**: When the __onSave__ event is not set in the __$.ig.AnalyticsView__ object, the default server-side saving method is used. After the end user saves a modified dashboard, a HTTP POST request is invoked. As a result, the __SaveDashboardAsync__ method of the currently defined SDK context is invoked. And you get the \_dashboardId as string and a Stream representation of the dashboard in **dashboardStream**.
