...
In the case that you don’t set your own function to this callback, when a new visualization is created, Analytics will display all data sources used in the dashboard (if any).
...
window.revealView.onDataSourcesRequested = function (callback) {
    ...
    callback(new $.ig.AnalyticsDataSources([sqlDs], [inMemoryDSI], false));
};
```
...
Creating dashboards from scratch is really simple, you just need to:
  ...
  - Initialize __$.ig.AnalyticsView__ object, without setting the dashboard property to $.ig.AnalyticsView and without using __$.ig.RVDashboard.loadDashboard__;
