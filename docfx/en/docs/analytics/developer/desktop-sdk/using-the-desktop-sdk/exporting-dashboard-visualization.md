...
To enable a dashboard or a visualization export, you can:
  ...
  - use the export setting [in the
    AnalyticsView](#enable-export-revealview), or

  - initiate export programmatically [outside of the
    AnalyticsView](#programmatically-initiated-export), when exporting **as an image**.

### Prerequisites for Export as an Image Option

To use the Export as an image feature you will need to add a reference to [CefSharp.Wpf NuGet package (\>= 83.4.20)](~/components/analytics/developer/setup-configuration/setup-configuration-desktop.md) to your project.
...
- __AnalyticsView.ShowExportImage__ - for export as an **image**;

- __AnalyticsView.ShowExportToPDF](api-reference__ - for export as a **PDF** document;

- __AnalyticsView.ShowExportToPowerpoint__ - for export as a **PowerPoint** presentation;

- __AnalyticsView.ShowExportToExcel__ - for export in **Excel** data format.
...
If they click the *Export Image* button on the bottom right, the AnalyticsView raises the __ImageExported__ event. To access the image via the __Image
Property__
...
``` csharp
private void AnalyticsView_ImageExported(object sender, ImageExportedEventArgs e)
{
  var image = e.Image;
...
To get an image of the AnalyticsView programmatically, you will need to invoke the __ToImage__ method. Calling this method will not result in showing the *Export Image* dialog. This way, you can get a screenshot when the user clicks a button, which is outside of the AnalyticsView. This method will create a screenshot of the AnalyticsView component as it is displayed on the screen.
