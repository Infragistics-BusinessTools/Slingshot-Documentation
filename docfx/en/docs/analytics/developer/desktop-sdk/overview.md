...
When you embed Analytics in another application (Windows WPF, Windows
Forms, iOS or Android), the Analytics SDK is provided as a library or
framework that is integrated into the app (the integration steps are
different for each platform).

### The AnalyticsView Component

The containing app creates a AnalyticsView object configured with the
...
The AnalyticsView component provides rendering and data transformation
capabilities (automatically through the Analytics Engine), but it does not
...
a secure storage. This means that Analytics delegates the storage and
