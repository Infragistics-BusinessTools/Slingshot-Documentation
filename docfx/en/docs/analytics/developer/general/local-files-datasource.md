...
When we create a dashboard in **Analytics Application** we often use Excel or CSV files stored in the cloud to populate it with data.   
After exporting the dashboard and embedding it in custom application, we can move these files in a local directory and then use the **Analytics SDK** to access and set them as a local datasource. 
...
[**Loading Dashboard Files**](~/components/analytics/developer/desktop-sdk/using-the-desktop-sdk/loading-dashboards.md) (for WPF), or 
[**Creating Your First App**](~/components/analytics/developer/web-sdk/create-first-app.md) (for Web)
...
6. **Set the *DataSourceProvider* property** of the *AnalyticsSdkContext* class to *CloudToLocalDatasourceProvider*:  
