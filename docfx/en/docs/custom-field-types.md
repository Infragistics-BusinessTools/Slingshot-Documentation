...
| ***Formula***| It can be used in tasks where users want to calculate a value by selecting two existing fields. Both fields must be numbers (add, subtract, multiply, or divide) or both dates (subtract to get the number of days between them). The result is read-only and updates when those fields change.| You are a campaign manager who tracks revenue and cost per campaign. You can add a **Margin** field that subtracts _Cost_ from _Revenue_, so every task shows the margin automatically.|
