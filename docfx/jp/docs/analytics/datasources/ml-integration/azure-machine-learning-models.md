...
2.  **表示形式エディター**で、左側の **[フィールド]** リストの上にある**脳アイコン**をクリックまたはタップします。次に、**Microsoft Azure** を選択します。
    ...
    <img src="images/azure-ml-models-brain-icon.png" alt="Brain icon location in the Visualization editor" class="responsive-img" width="80%"/>
...
2.  表示されるダイアログで、Analytics のデータを機械学習モデルで予期される入力にマップする必要があります。
    ...
    b. **[出力]**: モデルで計算 (予測) する情報を選択します。結果は、表示形式エディターの新しいフィールドとして表示されます (**データ ソース: Azure model** の下)。
      ...
      <img src="images/loaded-data-from-azure-ml-model.png" alt="Data output loaded from Azure model" class="responsive-img" width="40%"/>
