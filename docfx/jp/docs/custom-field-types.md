...
| ***数式***| 既存の 2 つのフィールドを選択して値を計算したいタスクで使用できます。両方のフィールドが数値（加算、減算、乗算、または除算）か、両方が日付（差を引いて期間の日数を取得）である必要があります。結果は読み取り専用で、それらのフィールドが変更されると自動的に更新されます。| あなたは、各キャンペーンの収益とコストを追跡するキャンペーン マネージャーであるとします。_コスト_ を _収益_ から差し引く **「マージン」** フィールドを追加できます。これにより、各タスクにマージンが自動的に表示されます。|
